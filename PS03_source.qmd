...
-   Wichita Falls is the last city listed in the data set.
:::
...
-   The listing variable in this data set is 'Total Active Listings.'
:::
...
dallas_sub <- txhousing |>
  filter(city == "Dallas", between(year, 2012, 2013))
 
dim(dallas_sub)
head(dallas_sub)


 dallas_sub2 <- txhousing |> 
  filter(city =="Dallas" & (year == 2012 | year == 2013))
...
dallas_summary <- dallas_sub |>
  group_by(month) |>
  summarize(mean_prct_sold = (mean(sales/listings * 100)))
...
-   The mean percentage of listings sold between 2012-2013 in Dallas, grouped by month, arranged in descending order.
:::
...
-   Midland had the fewest houses listed for sale at 0 listings.
:::
...
```{r}
# Type your code and comments inside the code chunk
Austin_Dallas <- txhousing |>
  filter((city == "Austin" | city == "Dallas") & (year == 2000 | year == 2001)) |> 
  group_by(city) |>
  summarize(total_houses_sold_00_01 = sum(sales))
